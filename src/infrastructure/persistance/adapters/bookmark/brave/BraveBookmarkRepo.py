...
class BraveBookmarkRepo(IBookmarkRepo):
	...
	def __init__(self, whitelist_repo: Union[IWhitelistRepo, None] = None, brave_bookmarks_path: Union[str, None] = None):
		...
		self.__whitelist = None if whitelist_repo is None else whitelist_repo.get_whitelist()
	...
	def __rec_parse__brave_bookmark(self, bookmark_or_folder: dict, current_path: str, only_parse_whitelisted: bool) -> Union[Folder, Bookmark, None]:
		...
		if bookmark_or_folder["type"] == "folder":
			...
			for child in bookmark_or_folder["children"]:
				parsed_child = self.__rec_parse__brave_bookmark(bookmark_or_folder=child, current_path=new_path, only_parse_whitelisted=only_parse_whitelisted)
	...
	def __rec_filter_folder(self, folder: Folder) -> Folder:
		...
		children = [self.__rec_filter_folder(child) if isinstance(child, Folder) else child for child in children]
	...
	def __parse_bookmarks(self, bookmarks_export: dict, only_parse_whitelisted:bool = True) -> Folder:
		...
		children = [self.__rec_parse__brave_bookmark(bookmark_or_folder=child, current_path="",only_parse_whitelisted=only_parse_whitelisted) for child in bookmarks_export["roots"]["bookmark_bar"]["children"]]
